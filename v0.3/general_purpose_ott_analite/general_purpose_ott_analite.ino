...
String take_measurement() {
  ...
  // IF OTT + ANALITE
  String ottString = sample_ott_M();
  int delimeter_1 = ottString.indexOf(",");
  int delimeter_2 = ottString.indexOf(",", delimeter_1 + 1);
  water_level_mm = round(ottString.substring(0, delimeter_1).toDouble()*1000);
  water_temp_c = ottString.substring(delimeter_1+1, delimeter_2).toDouble();
  ott_status = round(ottString.substring(delimeter_2+1, ottString.length()).toDouble());
  water_ntu = sample_analite_195().toDouble();

  String msmt = String(sample_batt_v()) + "," + 
    freeMemory() + "," + 
    String(water_level_mm) + "," + 
    water_temp_c + "," + 
    ott_status + "," + 
    sample_ott_V() + "," +
    water_ntu;
