...
const byte triggerPin = 10; //Range start / stop pin for MaxBotix MB7369 ultrasonic ranger (PIN 4 GREEN)
const byte pulsePin = 12; //Pulse width pin for reading pw from MaxBotix MB7369 ultrasonic ranger (PIN 2 ORANGE)
// US PWR =  RED, GND = BLACK
...
long sample_ultrasonic(){
  ...
  unsigned long duration = pulseIn(pulsePin, HIGH);  //G et the pulse duration (i.e.,time of flight)
  ...
  while(duration == 0){
    ...
    delay(1);    
    digitalWrite(triggerPin, LOW);  //Write the ranging trigger pin HIGH
    duration = pulseIn(pulsePin, HIGH);  //G et the pulse duration (i.e.,time of flight)
    }
  return duration;
  }
