...
const byte dataPin_ds18b20 = 11;  // The pin of the SDI-12 data bus
const byte dataPin_hydros = 12;   // The pin of the SDI-12 data bus
const byte IridSlpPin = 13;       // Power base PN222 2 transistor pin to Iridium modem

/*Define global vars */
String my_letter = "ABCB";
String my_header = "datetime,batt_v,memory,water_level_mm,water_temp_c,water_ec_dcm,water_temp_c_2";
int err;

// Setup a oneWire instance to communicate with any OneWire device
OneWire oneWire(dataPin_ds18b20);
...
SDI12 mySDI12(dataPin_hydros);                // Define the SDI-12 bus
QuickStats stats;                             // Instance of QuickStats

String sample_DS1820() {
  sensors.requestTemperatures();
  String temp2 = String(sensors.getTempCByIndex(0));
  // String temp3 = String(sensors.getTempCByIndex(1));
  // String temp4 = String(sensors.getTempCByIndex(2));
...
String take_measurement() {
  ...
  String msmt = String(sample_batt_v()) + "," + freeMemory() + "," + sample_hydros_M() + "," + sample_DS1820();
...
String prep_msg() {
  ...
  float *out_water_temp_c_2 = (float *)cp["water_temp_c_2"];
  // float *out_water_temp_c_3 = (float *)cp["water_temp_c_3"];
  // float *out_water_temp_c_4 = (float *)cp["water_temp_c_4"];

  String datastring_msg =
    my_letter + ":" + String(out_datetimes[0]).substring(2, 4) + 
    String(out_datetimes[0]).substring(5, 7) + 
    String(out_datetimes[0]).substring(8, 10) + 
    String(out_datetimes[0]).substring(11, 13) + ":" + String(round(out_batt_v[num_rows - 1] * 100)) + ":" + String(round(out_mem[num_rows - 1] / 100)) + ":";

  for (int i = 0; i < num_rows; i++) {  //For each observation in the IRID.csv
    datastring_msg =
      datastring_msg + String(round(out_water_level_mm[i])) + ',' + 
      String(round(out_water_temp_c[i] * 10)) + ',' + 
      String(round(out_water_ec_dcm[i])) + ',' + 
      String(round(out_water_temp_c_2[i] * 10)) + ':'; 
      // String(round(out_water_temp_c_3[i] * 10)) + ',' + 
      // String(round(out_water_temp_c_4[i] * 10)) + ':';
  }
...
void setup(void) {
  ...
  pinMode(dataPin_hydros, INPUT);
  pinMode(dataPin_ds18b20, INPUT);
...
int send_msg(String my_msg) {
  ...
  Serial.print("  - Sync clocks... ");
  Serial.println(err_time);
