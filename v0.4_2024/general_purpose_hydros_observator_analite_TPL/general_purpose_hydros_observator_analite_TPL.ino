...
const byte WiperPin = A4;    //Power relay set pin to HYDROS21
...
// /*Define global vars */
String my_letter = "ABCDD";
String my_header = "datetime,batt_v,memory,water_level_mm,water_temp_c,water_ec_dcm,nep_temp,nep_med,nep_mean,nep_min,nep_max,ntu,cor_msmt_ctd_stage_m,cor_msmt_ctd_wl_m";
int err; 
...
String prep_msg(){
  ...
  SD.begin(chipSelect);
  CSV_Parser cp("sfffffffffffff", true, ',');  // Set paramters for parsing the log file
  ...
  float *out_ntu_195 = (float *)cp["ntu"];
  ...
  for (int i = 0; i < num_rows; i++) {  //For each observation in the IRID.csv
    datastring_msg = 
      ...
      String(round(out_ntu_obs[i]*10)) + ',' + 
      String(round(out_ntu_195[i])) + ':';              
    }
...
void setup(void) {
  ...
  digitalWrite(WiperPin, LOW); delay(50);
...
\  String sdiResponse = "";  // Local response string
  ...
  Serial.println("OBS M6 command response: " + sdiResponse);
  
  // Clear buffer after reading response
  mySDI12_OBS.clearBuffer();
  
  // Wait for measurement to complete (this is a long measurement)
  Serial.println("Waiting for statistical measurement to complete... 70s...");
  delay(70000);
...
void wiper_analite_195() {
  ...
  digitalWrite(WiperPin, LOW);  // turn the LED on (HIGH is the voltage level)
  delay(100);
  digitalWrite(WiperPin, HIGH);  // turn the LED on (HIGH is the voltage level)
