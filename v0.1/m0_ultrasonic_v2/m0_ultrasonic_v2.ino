...
/*Define global vars */
String my_header = "datetime,water_level_mm,batt_v";
...
String sample_ultrasonic(){
  ...
  int16_t med_distance = (round(stats.median(values, n_samples))/58.0)*10;  //Copute median distance
  digitalWrite(triggerPin, LOW);  //Write the ranging trigger pin HIGH
  return String(med_distance);
}
...
String prep_msg(){
  ...
  SD.begin(chipSelect);
  CSV_Parser cp("sff", true, ',');  // Set paramters for parsing the log file
  ...
  read_params();
  String datastring_msg = "A:" + String(out_datetimes[0]).substring(2, 4) + String(out_datetimes[0]).substring(5, 7) + String(out_datetimes[0]).substring(8, 10) + String(out_datetimes[0]).substring(11, 13) + ":" + String(round(out_batt_v[num_rows-1] * 100)) + ":";
  
  for (int i = 0; i < num_rows; i++) {  //For each observation in the IRID.csv
    datastring_msg = datastring_msg + String(round(out_water_level_mm[i])) + ':';              
    }
...
void setup(void){
  ...
  pinMode(13, OUTPUT); digitalWrite(13, HIGH); delay(30); digitalWrite(13, LOW);  
  ...
  String datastring = rtc.now().timestamp()+","+sample_ultrasonic()+","+sample_batt_v();
...
void loop(void){
  ...
  if(present_time.second() % 10 == 0){
    ...
    if(present_time.minute() % sample_freq_m_32 == 0 & present_time.second() == 0){    
      ...
      // SAMPLE - SAMPLE 
      String datastring = present_time.timestamp()+","+String(sample_ultrasonic()+","+sample_batt_v());
      Serial.println(datastring);
